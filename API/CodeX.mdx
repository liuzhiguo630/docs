...
CodeX 计费方法同步官方，价格比例请参考 [模型与价格](/aggregation/Model-Price.mdx)
...
<Steps>
  ...
  <Step title="第三步 - 方法A：修改配置文件以持久化【推荐】">
      <Card title="编辑配置文件" img="/images/API/CodeX/step-3a.png">
        接着使用您的操作系统进入 `~/.codex` 路径，使用`vim`或其他工具编辑 `config.toml` 文件，添加如下配置：
        ```toml config.toml {2,3}
        model_provider = "liao"
        ...
        ```
        然后在`~/.codex`路径下新建`auth.json`文件，内容如下：
        ```json
        {
          "OPENAI_API_KEY": "您的登录码"
        }
        ```
      </Card>
    </Step>
  <Step title="第三步 - 方法B：配置环境变量">
    <Card title="编辑配置文件">
  ...
  <Step title="第四步">
    <Card title="开始使用" img="/images/API/CodeX/step-4.png">
      通过 cd 命令进入您想要编辑的项目文件夹，输入 `codex` 后就可以开始互动了
    </Card>
  </Step>
</Steps>